...
En hvað ef við þyrftum að meira bili að halda á milli línanna?
...
\item Hægt er að kalla á sama fallið aftur og aftur. Reyndar vill svo til að það er einmitt algengt og nytsamlegt.
...
Þett er einnig algengt og nytsamlegt. 
...
\item Með því að búa til nýtt fall þá getur þú gefið mengi af setningum nafn.
...
Sem betur fer þurfum við sem forritara ekki að hafa áhyggjur af þessum stökkvum í föll því C++ þýðandinn býr til kóða sem sér um þetta fyrir okkur.
...
Boðskapurinn er sem sagt sá að þeagar þú lest forritið þá skaltu ekki lesa það frá toppi til táar heldur fylgja keyrsluflæðinu (e. flow of execution).
...
Það er mikilvægt að gera sér grein fyrir að leppurinn og viðfangið eru ekki sami hluturinn
þó svo að þau hafi sama gildið (í þessu tilviki stafurinn {\tt 'b'}).

Gildið sem sent er sem viðfang verður að hafa sama tagið og leppurinn í fallinu sem kallað er á.
Þessi regla er mikilvæg en getur stundum verið ruglandi því C++ breytir stundum einu tagi í annað á sjálfvirkan hátt.
...
Gildissvið (e. scope) leppar og breytna eru aðeins innan í eigin föllum.
Innan marka {\tt main} er ekki neinn hlutur með nafninu {\tt phil} aðgengilegur.
Þýðandinn mun kvarta ef þú reynir að nota það nafn inna í {\tt main}.
...
Eins og stöðurit sýna staflarit gildi á sérhverri breytu en breyturnar eru innan í stærri boxum sem gefa til kynna hvaða föllum þær tilheyra.
...
\vspace{0.1in}
%
Í hvert skipti sem fall er kallað verður til nýtt {\bf tilvik} af fallinu (kallað {\bf kvaðningafærsla} (e. activation record)).
Sérhvert tilvik af fallinu innheildur leppana og staðværu breyturnar.
...
Í fyrsta lagi þarf að muna að það þarf að lýsa yfir tagi á sérhverjum leppi.
Dæmi:
\begin{verbatim}
...
\item Getum við skrifað föll sem skila gildum eða sitjum við upp með að skrifa föll eins og {\tt newLine} og {\tt printTwice}?
...
\item[kommutala (e. floating-point):] Tag breytu (eða gildi) sem getur geymt brot svo og heiltölur.
...
Gildið verður að vera af sama tagið og viðkomandi leppur.
