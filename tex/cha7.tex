...
The second line assigns it the string value \verb+"Hello."+
The third line is a combined declaration and assignment, also
called an initialization.

Normally when string values like \verb+"Hello, "+ or \verb+"world."+
...
\end{verbatim}
%
{\tt a} is not the first letter of \verb+"banana"+.  Unless you are a
computer scientist.  For perverse reasons, computer scientists always
start counting from zero.  The 0th letter (``zeroeth'') of \verb+"banana"+ is {\tt b}.
The 1th letter (``oneth'') is {\tt a} and the 2th (``twoeth'') letter is {\tt n}.
...
in \verb+"banana"+.  Since we started counting at 0, the 6
...
{\tt letter} + \verb+"ack"+ is syntactically legal in C++, although it
